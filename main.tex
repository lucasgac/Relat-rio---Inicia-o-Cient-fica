...
\begin{proposition}
    Isometrias preservam conexões de Levi-Civita. Mais ainda, mapas que preservamos conexões, preservam geodésicas. Como corolário, isometrias preservam geodésicas.
\end{proposition}
...
\begin{proposition}
    Se $V$ e $W$ são paralelos ao longo de $c$, então $||V||$ e $\mathfrak{g}(V, W)$ são constantes.
\end{proposition}

\subsection{Mapa exponencial e mapa logarítmico}

Uma geodésica $c \colon I \to M$ é \textit{maximal} se não podemos estender $c$ para um intervalo maior sem que a curva deixe de ser uma geodésica.
